...
class CardView11(
    ...
    private fun init(
        ...
        context.withStyledAttributes(attrs, R.styleable.CardView11) {
            for (index in 0 until indexCount) {
                when (val attr = getIndex(index)) {
                    R.styleable.CardView11_cv11_srcImage ->
                        setImageSrc(
                            getResourceId(attr, R.drawable.background_image),
                        )

                    R.styleable.CardView11_cv11_srcImageTint ->
                        setImageSrcTint(
                            getResourceId(attr, android.R.attr.colorControlHighlight),
                        )

                    R.styleable.CardView11_cv11_action1Drawable ->
                        setActionButton1Drawable(
                            getResourceId(attr, R.drawable.ic_star_outline),
                        )

                    R.styleable.CardView11_cv11_action2Drawable ->
                        setActionButton2Drawable(
                            getResourceId(attr, R.drawable.ic_star_outline),
                        )

                    R.styleable.CardView11_cv11_action3Drawable ->
